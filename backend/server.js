...
function getScreenScore(product) {
    ...
    const brightness =
        extractNumber(product.brightness);
    ...
    const resolution =
        String(product.resolution || "")
            .toLowerCase();

    const gamut =
        String(product.colorGamut || "")
            .toLowerCase();

    const screenType =
        String(product.screenType || "")
            .toLowerCase();
...
function getBatteryScore(product) {
    ...
    const battery =
        extractNumber(product.battery);

    const weight =
        extractNumber(product.weight);
...
function calculateScores(product) {
    ...
    const screen =
        getScreenScore(product);

    const battery =
        getBatteryScore(product);
...
app.post("/api/products", (req, res) => {
    ...
    /*
     * Application des éventuelles
     * valeurs modifiées manuellement.
     */
    ...
    /*
     * Si aucun score manuel n'a été défini,
     * on ne stocke pas d'objet inutile.
     */
...
app.put("/api/products/:id", (req, res) => {
    ...
    /*
     * Les scoreOverrides envoyés par l'interface
     * sont ajoutés aux overrides déjà existants.
     */
    ...
    /*
     * On remplace le produit dans la base.
     */
